...
class MyAdapter(private val items: RandomUser, val context: Context) :
    ...
    override fun onBindViewHolder(holder: ViewHolderRandom, position: Int) {
        ...
        holder.randomUserAddress.text =
            items.results[position].location.street.number.toString() + " " + items.results[position].location.street.name + ", " + items.results[position].location.city + ", " + items.results[position].location.country
        holder.randomUserEmail.text = items.results[position].email
        Picasso.with(context).load(items.results[position].picture.large).into(holder.randomUserPict)
    }
